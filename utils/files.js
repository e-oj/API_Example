...
export const getImg = async (req, res) => {
  ...
  if(!req.query["id"]){
    return respondErr(HTTP_STATS.BAD_REQUEST, "Missing required id");
  }
  ...
  readStream.on("error", err => {
    respondErr(HTTP_STATS.BAD_REQUEST, err.message);
  });
 ...
 * @returns {Promise.<*>}
 */
export const toMp4 = (file, maxDuration = MAX_DURATION) => {
 ...
 * @returns {Promise.<*>}
 */
export const uploadVideo = async (file, maxDuration = MAX_DURATION) => {
  let vid;
...
export const uploadVideos = async (files, maxDuration) => {
  let result = [];
  let res;
...
export const stream = async (req, res) => {
  ...
  try{
    ...
    if (!fileArr.length) {return respondErr(HTTP_STATS.NOT_FOUND, "File not found");}
    ...
    if (byteRange === -1) {
      return respondErr(HTTP_STATS.UNSATISFIABLE_RANGE, "Invalid Range");
    }

    res.set("Content-Type", mimeType);
    res.set("Accept-Ranges", "bytes");
    res.status(HTTP_STATS.PARTIAL_CONTENT);
    ...
    if(byteRange.type !== "bytes"){
      return respondErr(HTTP_STATS.UNSATISFIABLE_RANGE, "Bytes only, please!!");
    }
    ...
    respondErr(HTTP_STATS.SERVER_ERROR, err.message, err);
  }
};
